...
class NeuMFWithBERT(nn.Module):
    def __init__(self, num_users, num_items, item_bert_matrix, mf_dim=64, mlp_dims=[128, 64]):
        ...
        self.item_bert_matrix = nn.Parameter(torch.tensor(item_bert_matrix, dtype=torch.float32), requires_grad=False)

        total_input = mf_dim + mlp_dims[1] + 768
    ...
    def forward(self, user_idx, item_idx):
        ...
        bert_emb = self.item_bert_matrix[item_idx]  # [batch, 768]

        final_input = torch.cat([gmf, mlp_out, bert_emb], dim=-1)
