...
class NeuMFWithBERT(nn.Module):
    def __init__(self, num_users, num_items, item_bert_matrix, user_matrix,
                 mf_dim=64, mlp_dims=[128, 64]):
        super().__init__()
        self.mf_dim = mf_dim
        self.mlp_dims = mlp_dims
        self.bert_item_dim = item_bert_matrix.shape[1]
        self.bert_user_dim = user_matrix.shape[1]
        ...
        self.user_bert_matrix = nn.Parameter(
            torch.tensor(user_matrix, dtype=torch.float32),
            requires_grad=False
        )

        # MLP input: user_id_emb + item_id_emb + item_BERT + user_BERT
        mlp_input_dim = mlp_dims[0] + self.bert_item_dim + self.bert_user_dim
        self.mlp = nn.Sequential(
            nn.Linear(mlp_input_dim, mlp_dims[1]),
            nn.ReLU(),
            # nn.Dropout(0.2)
        )

        # Final concat of GMF + MLP output
        final_input_dim = mf_dim + mlp_dims[1]
        self.output = nn.Linear(final_input_dim, 1)
    ...
    def forward(self, user_idx, item_idx):
        ...
        # MLP branch
        user_id_emb = self.user_embedding_mlp(user_idx)
        item_id_emb = self.item_embedding_mlp(item_idx)
        item_bert_emb = self.item_bert_matrix[item_idx]  # [batch, 768]
        user_bert_emb = self.user_bert_matrix[user_idx]  # [batch, user_bert_dim]

        mlp_input = torch.cat([user_id_emb, item_id_emb, item_bert_emb, user_bert_emb], dim=-1)
...
if __name__ == "__main__":
    ...
    user_matrix = np.load("user_bert4rec_embeddings.npy")
    # Step 2: Initialize the fused model
    model = NeuMFWithBERT(num_users, num_items, bert_matrix, user_matrix).to(device)
